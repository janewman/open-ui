...
| [`wrap`](#enabling-linear-wrapping-behavior) | Optional | Linear: loops end→start and start→end. Grid: wraps within the same row and column (`row-wrap col-wrap`). Some behavior tokens enable `wrap` by [default](#default-modifiers). |
| [`nowrap`](#enabling-linear-wrapping-behavior) | Optional | Explicitly hard edges. Overrides a behavior token's default `wrap`, and on a grid overrides `wrap`/`flow`. Cannot be combined with `wrap`. |
...
The `grid` behavior derives its 2-D topology from HTML structure — a native table for automatic mode, or `focusgrouprow`-marked rows for [`manual`](#manual-row-enrollment) mode. Like the linear behaviors, it infers minimum roles one-way (`grid` on a generic owner, `row` on a generic marked row, and `gridcell` on a generic cell) under the [precedence rules](#behavior--role-mapping-and-precedence) below. Markup that cannot form a rectangular grid follows the [invalid-grid fallback](#invalid-grid-structures).
...
- The `feed` and `grid` behaviors carry `itemcontrols` as a default modifier; a redundant explicit `itemcontrols` remains valid.
...
are `focusgroup`, `focusgroupstart`, and `focusgrouprow` (lowercase).
...
The `feed` wrapping and key model remains open, so the current draft accepts neither wrapping token for that behavior.
...
Grid topology comes from HTML structure. Automatic mode reads a native table (`<tr>`, `<td>`/`<th>`, and native row groups); manual mode reads `focusgrouprow` markers and each marked row's direct element children. The grid then infers `grid`, `row`, and `gridcell` roles one-way on generic elements, following the shared [minimum-role precedence](#behavior--role-mapping-and-precedence). A native `<table>` continues to map through HTML-AAM; authors add `role="grid"` when they want the composite `grid` role exposed. The initial model does not support `treegrid`.
...
`focusgrouprow` is structural: it never establishes a focusgroup owner or definition, nested scope, focusgroup segment, or memory. On a generic element it contributes a one-way inferred `row` role under the shared [minimum-role precedence](#behavior--role-mapping-and-precedence). Cells are not marked with a `focusgroupcell` attribute: each direct element child of a marked row is a cell coordinate, and a generic cell is exposed one-way as `gridcell`.
