...
V1 linear focusgroup behavior tokens (APG alignment and minimum roles). All six produce the same linear navigation semantics — only semantic expectations, child inference, and default modifiers differ. The provisional `feed` row defines no inferred roles, and the V2 grid row is retained for the detailed grid design below:
...
| `feed` (proposed; working name) | (none; provisional interactive list) | (none) | (none) | `itemcontrols` | [Proposed feed focusgroups](#proposed-feed-focusgroups) |
...
Current focus therefore makes an otherwise non-sequentially-focusable element, including one with negative `tabindex`, a boundary for that navigation step.

When excluded content meeting this condition occurs between two focusgroup items, it divides the focusgroup into one segment on each side. Directional navigation may cross segments while skipping the excluded content; segments constrain sequential entry rather than directional reachability.
...
The proposed `itemcontrols` changes to entry, sequential navigation, and memory are defined in the [`itemcontrols` extension](#proposed-sequential-entry-extension).
...
- Each focusable `group` is an outer focusgroup item representing one labeled layer. The block axis moves directional navigation between groups and makes the target group active.
...
- Shift+Tab from the first associated button returns focus to the active group boundary. The focusgroup continues to remember that group as its active item.
