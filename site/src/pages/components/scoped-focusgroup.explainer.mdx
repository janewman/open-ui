...
| [`itemcontrols`](#v2-itemcontrols) | V2 modifier; implied by `feed` | Would make `focusgroup="none"` descendants of the active item available to sequential focus navigation while skipping those associated with inactive items. |
| [`feed`](#v2-feed-focusgroups) | V2 interactive-list behavior; working name | Would imply `itemcontrols`; its key and semantic model remain open. |

## Focusgroup V1 and V2

Focusgroup **V1** is the current linear design. It narrows the scope of the [original focusgroup explainer](https://github.com/openui/open-ui/blob/fdd348a/site/src/pages/components/focusgroup.explainer.mdx). **V2** covers the active new work — grid, `itemcontrols`, and `feed`. This draft also proposes the [`itemcontrols`](#v2-itemcontrols) modifier for controls inside an active item and the provisional [`feed`](#v2-feed-focusgroups) interactive-list behavior, whose final name and key model remain open.
...
| `feed` (V2; working name) | (none; provisional interactive list) | (none) | (none) | `itemcontrols` | [V2 feed focusgroups](#v2-feed-focusgroups) |
...
- **Guaranteed tab stop:** Each segment ensures exactly one tab-accessible focusgroup item. The [`itemcontrols`](#v2-itemcontrols) extension would additionally expose `focusgroup="none"` controls associated with the active item.
...
The selected behavior determines outer items before `itemcontrols` changes sequential navigation. Linear behaviors use their managed focusgroup items; the `feed` behavior uses its top-level semantic list items. Each opted-out control is then associated with its nearest containing outer item.
...
Under this model, the focusgroup retains memory of the active outer item while focus moves through its opted-out controls. Whether `itemcontrols` should also remember the last control, and how directional navigation resumes directly from a control, remain open design questions.
...
The behavior currently named `feed` is a candidate for vertically ordered interactive lists whose items may contain nested controls, such as chat threads, notification lists, file lists, and cards with actions. It would imply [`itemcontrols`](#v2-itemcontrols), keeping opted-out controls associated with the active item available through sequential focus navigation while skipping controls associated with inactive items.
...
The following markup illustrates the item and `itemcontrols` structure only. It is not a complete keyboard pattern, because the directional key model is unresolved.
...
***V2 interactive list and item controls:***
