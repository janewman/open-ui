...
The `grid` and `grid-manual` behaviors define no minimum role mapping or row/cell inference. Their owners must already expose compatible `grid` semantics, and their rows and cells must already expose the required semantics through native table context, explicit ARIA roles, or custom-element default semantics. Markup that does not meet those requirements follows the [invalid-grid fallback](#invalid-grid-structures).
...
| (automatic grid default) `focusgroup="grid"` | grid | No wrapping; row and column edges are hard stops. |
| (manual grid default) `focusgroup="grid-manual"` | grid | No wrapping; row and column edges are hard stops. |

Specifying both `wrap` and `nowrap` in one HTML `focusgroup` definition is an author error.

The proposal does not define `wrap` or `nowrap` for `grid` or `grid-manual`; their boundaries are hard stops. The `feed` wrapping and key model remains open, so the current draft accepts neither wrapping token for that behavior. At a grid edge, the user agent does not consume the key for grid focusgroup behavior.
...
|---|---|---|
| Behavior axis default, or both directions when the behavior has no axis default | (unspecified value) | Linear behaviors |
| Both axes derived from semantic row and column topology | (no direction modifier) | `grid`, `grid-manual` |
| Inline direction | `inline` | Linear behaviors |
| Block direction | `block` | Linear behaviors |

***Focusgroup wrapping:***

| Description | HTML syntax | Applies to |
|---|---|---|
| Behavior default | (unspecified value) | Linear and grid behaviors; the `feed` wrapping model remains open |
| no wrap (explicit, overrides default) | `nowrap` | Linear behaviors |
| wrap | `wrap` | Linear behaviors |

The `grid` and `grid-manual` behaviors do not accept `wrap` or `nowrap`. The `feed` wrapping model remains open; the current draft accepts neither wrapping token.
