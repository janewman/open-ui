...
Focusgroup **V1** is the current linear design. It narrows the scope of the [original focusgroup explainer](https://github.com/openui/open-ui/blob/fdd348a/site/src/pages/components/focusgroup.explainer.mdx). **V2** covers the active new work — grid, `itemcontrols`, and `feed`. This draft also proposes the [`itemcontrols`](#proposed-itemcontrols) modifier for controls inside an active item and the provisional [`feed`](#proposed-feed-focusgroups) interactive-list behavior, whose final name and key model remain open.
...
The proposed [`itemcontrols`](#proposed-itemcontrols) modifier changes how a focusgroup treats controls inside an item; it does not define another navigation topology.
...
- **Guaranteed tab stop:** Each segment ensures exactly one tab-accessible focusgroup item. The proposed [`itemcontrols`](#proposed-itemcontrols) extension would additionally expose `focusgroup="none"` controls associated with the active item.
...
The proposed [`itemcontrols`](#proposed-itemcontrols) extension defines additional sequential access separately.
...
The proposal would add the `itemcontrols` modifier for this behavior. Authors mark controls inside an outer item with `focusgroup="none"`, excluding them from directional navigation. `itemcontrols` would make those opted-out controls sequentially reachable only when their nearest containing outer item is active. The modifier could be used with any behavior.
...
The selected behavior determines outer items before `itemcontrols` changes sequential navigation. Shipped linear behaviors use their managed focusgroup items; the proposed `feed` behavior uses its top-level semantic list items. Each opted-out control is then associated with its nearest containing outer item.
...
- Each focusable row (`tabindex="0"`) is an outer focusgroup item. The block axis moves directional navigation between rows and makes the target row active.
- `focusgroup="none"` keeps each button out of directional navigation. Only the buttons associated with the active row participate in sequential focus navigation.
- Tab from the active row moves into its first associated button. Sequential navigation then follows the associated buttons in their normal order.
- Shift+Tab from the first associated button returns focus to the active row boundary. The focusgroup continues to remember that row as its active item.
- If focus is on `#more-background`, one Tab press moves focus to the search input. The inactive **Foreground** row and its opted-out buttons are not considered for that sequential navigation step.
- In an equivalent `focusgroup="toolbar block"` without `itemcontrols`, the opted-out buttons remain in sequential order and split the toolbar into segments. Tab from `#more-background` would continue into the next segment (the **Foreground** row and its buttons) instead of skipping to the search field.
...
The behavior currently named `feed` is a candidate for vertically ordered interactive lists whose items may contain nested controls, such as chat threads, notification lists, file lists, and cards with actions. It would imply [`itemcontrols`](#proposed-itemcontrols), keeping opted-out controls associated with the active item available through sequential focus navigation while skipping controls associated with inactive items.

The working name reflects an adjacent use case, not an automatic ARIA role mapping. This proposal defines no automatic `feed` or `article` role mapping or inference. Its final name, directional key set, wrapping behavior, loading model, and relationship to ARIA feeds all remain open.

If a true ARIA-feed behavior is considered later, it must build on existing native or explicit `article` semantics for each feed item. It must never infer `article` onto generic descendants, and it must not reuse the native `<button>` child-role exception for structural roles. Native buttons and links inside an article remain controls within that article, not peer feed items.

### Items and nested controls

Under the current working model, a top-level item is a focusable element whose native or explicit `listitem` semantics are owned by the `focusgroup="feed"` list. An item in a nested list belongs to that nested list and is not promoted to a peer of the top-level items; nested lists therefore act as boundaries. Generic or presentational wrappers may intervene only when they preserve that semantic ownership. Authors mark nested controls with `focusgroup="none"` so they do not become top-level items. Because `feed` implies [`itemcontrols`](#proposed-itemcontrols), only opted-out controls associated with the active item participate in sequential focus navigation.

No directional key behavior is specified yet. Native list semantics commonly keep assistive technologies in a reading mode where Up and Down are reading commands, so the proposal must resolve an assistive-technology-compatible key model before `feed` can define directional navigation.

The following markup illustrates the proposed item and `itemcontrols` structure only. It is not a complete keyboard pattern, because the directional key model is unresolved.
...
- The `<ul>` and `<li>` elements retain their native list semantics. The `feed` behavior adds no `feed` or `article` role.
- Each focusable top-level `<li>` is an interactive-list item; items in a nested list would not become peers.
- `feed` implies `itemcontrols`, so only the opted-out buttons associated with the active item participate in sequential focus navigation.
- Tab from the active list item moves into its first associated button; Shift+Tab from that button returns to the active list item.
- If focus is on `#comment-foo-bar` (the last associated control of the active **Foo Bar** item), one Tab press moves focus to the following search field. The inactive **Baz Bo** item and its opted-out buttons are skipped for that sequential step.
- Loading additional posts, activating a link, and opening an actions menu remain author behavior.

### Relationship to ARIA feeds

ARIA `feed` is a structural container rather than a composite widget. Assistive technologies commonly remain in reading mode and move focus between articles with reading commands. The APG feed convention uses Page Up and Page Down to move between articles and Ctrl+Home and Ctrl+End to move between the feed and the surrounding page; it does not take over Up and Down. Because the current proposal maps no roles and defines no directional keys, authors should not treat `focusgroup="feed"` as an ARIA feed until the interactive-list design resolves whether the token is renamed or gains a key model that preserves the ARIA feed contract.

### Dynamic content

When items are inserted, removed, hidden, disabled, or made inert, a `feed` focusgroup would update its item set and apply the [active-item invalidation](#active-item-invalidation) rules before the next relevant sequential operation. Virtualized interactive lists would expose only their rendered items to focusgroup navigation; focusgroup would neither request nor load additional content. Whether the proposal should define a separate ARIA-feed key and loading model remains an [open question](#open-questions).
...
***Proposed interactive list and item controls:***

| Description | HTML syntax |
|---|---|
| Interactive-list behavior; implies `itemcontrols`; key model open (working name) | `feed` |
| Active-item opted-out controls in sequential navigation (working name) | `itemcontrols` |
