...
Note that only the element with `id=one` can be focused using the Tab key. Because it is the first sequentially focusable element in the focusgroup, it becomes the single tab stop for the segment, following the rule that each focusgroup segment has exactly one tab stop.
...
Note that even if multiple descendants are naturally tabbable (e.g., several `<button>`s without an explicit `tabindex` attribute), `focusgroup` still collapses the group to a single sequential entry point per segment. Authors do NOT need to manually set `tabindex="-1"` on every non-active item to gain roving behavior.
...
#### 3. **Apply focus priority (first match wins):**
  ...
  - Otherwise, if navigating **forward**:
    - Focus the **first** item in the segment that is sequentially focusable (natively focusable or `tabindex >= 0`).
    - If no such item exists, focus the **first** focusable item in the segment (including those with a negative `tabindex`).
  - Otherwise (if navigating **backward**):
    - Focus the **last** item in the segment that is sequentially focusable (natively focusable or `tabindex >= 0`).
    - If no such item exists, focus the **last** focusable item in the segment (including those with a negative `tabindex`).

#### 4. **Update Sequential Focus Navigation:**
...
**Note on `tabindex > 0`**: While the algorithm respects a non-negative `tabindex` (`>=0`), the use of a positive `tabindex` is strongly discouraged as it disrupts the natural tab order of the page. Within a focusgroup, a positive `tabindex` is treated no differently than `tabindex="0"`.
