...
A `focusgrouprow` marker on an element with non-generic, non-`row` semantics, a malformed row or cell relationship, multiple eligible descendants in a nonfocusable cell, a focusable non-target descendant, an incompatible owner role, or an unsupported shape (ragged, spanning, or otherwise non-rectangular) makes the grid invalid. In that state, **grid focusgroup behavior is ignored**: directional navigation, memory, sequential-entry collapse, segment behavior, and any focusgroup inference for that grid do not run.
...
| Description | HTML syntax | Applies to |
|---|---|---|
| Behavior default (hard edges on a grid) | (unspecified value) | Linear behaviors and `grid`; the `feed` wrapping model remains open |
