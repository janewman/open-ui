...
| [`itemcontrols`](#v2-itemcontrols) | V2 modifier; implied by `feed` | Makes `focusgroup="none"` descendants of the active item available to sequential focus navigation while skipping those associated with inactive items. |

## Focusgroup V1 and V2

Focusgroup **V1** is the current linear design. It narrows the scope of the [original focusgroup explainer](https://github.com/openui/open-ui/blob/fdd348a/site/src/pages/components/focusgroup.explainer.mdx). **V2** covers the active new work — grid, `itemcontrols`, and `feed`. This draft also defines the [`itemcontrols`](#v2-itemcontrols) modifier for controls inside an active item and the [`feed`](#v2-feed-focusgroups) behavior for ARIA-feed-style content streams.
...
The `itemcontrols` modifier defines this behavior. Authors mark controls inside an outer item with `focusgroup="none"`, excluding them from directional navigation. `itemcontrols` makes those opted-out controls sequentially reachable only when their nearest containing outer item is active. The modifier can be used with any behavior and does not imply focus trapping, a nested focusgroup, or ARIA virtual focus.

### Active outer item

An `itemcontrols` focusgroup maintains an **active outer item** while focus is within the group. This state is distinct from the optional last-focused memory:

- Directional navigation to an outer item makes that item active.
...
- Sequential entry chooses an outer item using the normal priority order: remembered outer item, `focusgroupstart`, first sequentially focusable outer item, then first outer item.
- Forward entry focuses the chosen outer-item boundary. Reverse entry focuses its last associated opted-out control, when present, while keeping the outer item active.
- `nomemory` disables persistence after focus leaves the group, but not the active state needed while focus remains inside it. A later entry chooses a new outer item using the no-memory entry rules.
...
If the active outer item is removed, hidden, disabled, made inert, or otherwise stops being an eligible outer item, the focusgroup clears its active-item state and any memory that refers to it. `itemcontrols` does not move focus solely because of the mutation; focus disposition follows the underlying HTML rules.

If focus remains on a descendant that has another eligible owning outer item after the mutation, that outer item becomes active. Otherwise, the focusgroup has no active item until the next directional or sequential entry operation selects one. Owned controls from the invalidated item are no longer exposed through the `itemcontrols` extension. Independently scoped, opted-out, and top-layer descendants retain their existing behavior.
...
| Active-item opted-out controls in sequential navigation | `itemcontrols` |
