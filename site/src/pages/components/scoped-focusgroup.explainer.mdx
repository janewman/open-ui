...
For a `grid` focusgroup, the row and column coordinates follow [order-modified document order](https://drafts.csswg.org/css-display-4/#order-modified-document-order) — the visual order produced by CSS `order` — further adjusted by `reading-flow` and `reading-order` when present. Grid ownership still comes from HTML flat-tree ancestry: `order` and `reading-flow` never change which element is a row or cell, only the sequence of rows and of cells within a row. `order` affects visual order and `reading-flow` adjusts reading and sequential order; the grid intentionally uses their combined result for its directional coordinates, so inline and block movement, the row and grid extents, row-major traversal, and wrap or flow all track the visual layout.
