...
- [Wrap-around semantics](#enabling-wrapping-behaviors) - what to do when attempting to move past
   ...
   not moved past the ends of a `focusgroup` with the arrow keys. The `grid` behavior defaults to hard edges.
...
The proposal does not define `wrap` or `nowrap` for the `grid` behavior; its boundaries are hard stops. The `feed` wrapping and key model remains open, so the current draft accepts neither wrapping token for that behavior. At a grid edge, the user agent does not consume the key for grid focusgroup behavior.
...
The initial shape has no missing coordinate slots because every row has the same number of cells. Inline movement never crosses a row, and the `grid` behavior does not support `wrap` or `nowrap`. At a hard edge or when no eligible target exists in the requested direction, focus does not move and the user agent does not consume the key for grid focusgroup behavior.
...
The `grid` behavior does not accept `wrap` or `nowrap`. The `feed` wrapping model remains open; the current draft accepts neither wrapping token.
