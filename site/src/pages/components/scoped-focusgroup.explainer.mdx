...
| [`itemcontrols`](#proposed-itemcontrols) | Proposed modifier | Would make `focusgroup="none"` descendants of the active item available to sequential focus navigation while skipping those associated with inactive items. |

## Focusgroup V1 and V2

Focusgroup **V1** is the current linear design. It narrows the scope of the [original focusgroup explainer](https://github.com/openui/open-ui/blob/fdd348a/site/src/pages/components/focusgroup.explainer.mdx). **V2** covers the active new work — grid, `itemcontrols`, and `feed`. This draft proposes the [`itemcontrols`](#proposed-itemcontrols) modifier for controls inside an active item; it does not otherwise define an interactive-list behavior.
...
The proposed `itemcontrols` behavior adds a scoped exception for `focusgroup="none"` descendants of an outer focusgroup item. Those descendants remain excluded from directional navigation, but the controls associated with the active outer item participate in sequential focus navigation. Equivalent controls associated with inactive outer items do not create sequential entry points.
...
- [`itemcontrols`](#proposed-itemcontrols) - preserve an outer focusgroup item as the directional-navigation target while managing sequential access to its `focusgroup="none"` controls.
...
The selected behavior determines outer items before `itemcontrols` changes sequential navigation. Shipped linear behaviors use their managed focusgroup items. Each opted-out control is then associated with its nearest containing outer item.
