...
| [`wrap`](#enabling-wrapping-behaviors) | Optional | Linear: loops end→start and start→end. Grid: wraps within the same row and column (`row-wrap col-wrap`). Some behavior tokens enable `wrap` by [default](#default-modifiers). |
| [`nowrap`](#enabling-wrapping-behaviors) | Optional | Explicitly hard edges. Overrides a behavior token's default `wrap`, and on a grid overrides `wrap`/`flow`. Cannot be combined with `wrap`. |
...
| [`grid`](#v2-grid-focusgroups) | V2 2-D behavior | Provides 2-D directional navigation over a native HTML table structure. Pair with [`manual`](#manual-row-enrollment) for generic markup. |
| [`manual`](#manual-row-enrollment) | V2 grid modifier | Used as `focusgroup="grid manual"`. Enrolls every semantic row with the `focusgrouprow` attribute instead of discovering rows from a native table. |
| [`flow`, `row-wrap`, `col-wrap`, `row-flow`, `col-flow`](#grid-wrapping-and-flow) | Optional grid modifiers | Configure per-axis grid edges: wrap within the same row/column, or flow into the adjacent row/column. |
| [`itemcontrols`](#v2-itemcontrols) | V2 modifier; implied by `feed` and `grid` | Adds controls associated with the active outer item to sequential focus navigation and skips controls associated with inactive items. |

`focusgroupstart` and `focusgrouprow` are boolean structural companion attributes, not `focusgroup` tokens. Each marks an individual element and is reflected by an IDL property:
...
The `grid` behavior derives its 2-D topology from HTML structure — a native table for automatic mode, or `focusgrouprow`-marked rows for [`manual`](#manual-row-enrollment) mode — never from authored ARIA roles or `ElementInternals`. Like the linear behaviors, it infers minimum roles one-way (`grid` on a generic owner, `row` on a generic marked row, and `gridcell` on a generic cell) under the [precedence rules](#behavior--role-mapping-and-precedence) below, and it never overrides explicit or non-generic native semantics. Markup that cannot form a rectangular grid follows the [invalid-grid fallback](#invalid-grid-structures).
...
Behavior tokens and their APG alignment, minimum roles, and default modifiers. The six V1 tokens produce the same linear navigation semantics; the V2 `feed` and `grid` tokens add the ARIA feed structure and 2-D grid navigation. The Version column marks V1 (the current linear design) and V2 (active new work):

| Behavior | Version | APG Pattern | Minimum container role (when applied) | Minimum child role(s) (when applied) | Default modifiers | APG Pattern Link |
|---|---|---|---|---|---|---|
| `toolbar` | V1 | Toolbar | [`toolbar`](https://www.w3.org/TR/wai-aria-1.2/#toolbar) | (none) | `inline` | [APG Toolbar](https://www.w3.org/WAI/ARIA/apg/patterns/toolbar/) |
| `tablist` | V1 | Tabs | [`tablist`](https://www.w3.org/TR/wai-aria-1.2/#tablist) | [`tab`](https://www.w3.org/TR/wai-aria-1.2/#tab) | `inline` `wrap` | [APG Tabs](https://www.w3.org/WAI/ARIA/apg/patterns/tabs/) |
| `radiogroup` | V1 | Radio Group | [`radiogroup`](https://www.w3.org/TR/wai-aria-1.2/#radiogroup) | [`radio`](https://www.w3.org/TR/wai-aria-1.2/#radio) | `wrap` | [APG Radio Group](https://www.w3.org/WAI/ARIA/apg/patterns/radio/) |
| `listbox` | V1 | Listbox | [`listbox`](https://www.w3.org/TR/wai-aria-1.2/#listbox) | [`option`](https://www.w3.org/TR/wai-aria-1.2/#option) | `block` | [APG Listbox](https://www.w3.org/WAI/ARIA/apg/patterns/listbox/) |
| `menu` | V1 | Menu | [`menu`](https://www.w3.org/TR/wai-aria-1.2/#menu) | [`menuitem`](https://www.w3.org/TR/wai-aria-1.2/#menuitem) | `block` `wrap` | [APG Menu / Menubar](https://www.w3.org/WAI/ARIA/apg/patterns/menu/) |
| `menubar` | V1 | Menubar | [`menubar`](https://www.w3.org/TR/wai-aria-1.2/#menubar) | [`menuitem`](https://www.w3.org/TR/wai-aria-1.2/#menuitem) | `inline` `wrap` | [APG Menu / Menubar](https://www.w3.org/WAI/ARIA/apg/patterns/menu/) |
...
| [`grid`](#v2-grid-focusgroups) | V2 | [Grid](https://www.w3.org/WAI/ARIA/apg/patterns/grid/) | [`grid`](https://www.w3.org/TR/wai-aria-1.2/#grid) | [`row`](https://www.w3.org/TR/wai-aria-1.2/#row), [`gridcell`](https://www.w3.org/TR/wai-aria-1.2/#gridcell) | `itemcontrols` | [APG Grid](https://www.w3.org/WAI/ARIA/apg/patterns/grid/) |

The `manual` modifier (`focusgroup="grid manual"`) and the `itemcontrols` modifier are not behaviors and do not appear in this table. `manual` selects the grid's [manual row enrollment](#manual-row-enrollment); `itemcontrols` is described in [V2 itemcontrols](#v2-itemcontrols).
...
- The `feed` and `grid` behaviors carry `itemcontrols` as a default modifier, so authors need not spell it; a redundant explicit `itemcontrols` remains valid.
...
7. The `grid` behavior applies the same one-way, generic-only inference to a 2-D structure: a generic grid owner is exposed as `grid`, a generic `focusgrouprow` row under a [`manual`](#manual-row-enrollment) grid is exposed as `row`, and a generic cell identified by the grid's HTML topology is exposed as `gridcell`. Explicit or non-generic native semantics are preserved and are never overridden.
8. Grid topology comes from HTML structure, never from authored ARIA or `ElementInternals`: automatic mode reads the native table structure (`<tr>`, `<td>`/`<th>`, and native row groups), and manual mode reads `focusgrouprow` markers and each marked row's direct flat-tree element children. Explicit roles, when present, are preserved by this precedence but do not define topology.
...
To enable feature detection, user agents expose the `focusGroup`, `focusGroupStart`,
and `focusGroupRow` DOM properties on elements. Their content-attribute counterparts
are `focusgroup`, `focusgroupstart`, and `focusgrouprow` (lowercase). All three
companion surfaces are reflected together:
...
not validate a particular owner's roles, descendants, topology, or targets. A user
agent that recognizes `grid` but not `manual` ignores the unsupported `manual`
modifier under the ordinary [token-list fallback](#index-of-focusgroup-values), so
authors who depend on manual topology should test `supports('manual')` before using
`focusgroup="grid manual"`.
...
- [Wrap-around semantics](#enabling-wrapping-behaviors) - what to do when attempting to move past
   ...
   not moved past the ends of a `focusgroup` with the arrow keys. The `grid` behavior defaults to hard edges and accepts per-axis [wrap and flow modifiers](#grid-wrapping-and-flow).
- [Directional axis limits](#limiting-linear-focusgroup-directionality) - applies to linear
   ...
   respond to all four arrow keys. The `grid` behavior derives both axes from its topology.
...
The `grid` behavior has its own per-axis wrapping and flow modifiers, described in [Grid wrapping and flow](#grid-wrapping-and-flow). The `feed` wrapping and key model remains open, so the current draft accepts neither wrapping token for that behavior.
...
- **Hard edge** (the default) stops movement. Focus does not move and the user agent does not consume the key.

Wrapping and flow scan through [targetless cells](#cell-coordinates-and-directional-targets) exactly as ordinary movement does, stopping on the first eligible target. If a scan returns to the starting target without finding another eligible target, focus does not move and the key is not consumed.
...
```html
<div focusgroup="grid manual flow" aria-label="Emoji picker">
  <div focusgrouprow>
    <div tabindex="0">😀</div>
    <div tabindex="0">😉</div>
  </div>
...
- Moving inline-end from `😉` (last cell of the first row) continues at `😊` (first cell of the next row).
- Moving inline-end from `😎` (last cell of the last row) wraps around to `😀` (first cell of the first row).
...
Note that the following only apply to linear `focusgroup` definitions. The `grid` behavior derives both axes from row and column structure and does not accept `inline` or `block`; the `feed` key model remains open.
...
The `grid` behavior provides 2-dimensional directional navigation for static structures whose rows and columns have semantic meaning. Inline-direction commands move among eligible targets in one row. Block-direction commands preserve the current column while moving among rows.
...
`manual` is a grid modifier, not a separate behavior. It is scoped to `grid` today and reserved for reuse by future behaviors. An owner that does not include the `grid` behavior token ignores `manual` under the ordinary [token-list fallback](#index-of-focusgroup-values).

Grid topology comes from HTML structure, never from authored ARIA roles or `ElementInternals`. Automatic mode reads a native table (`<tr>`, `<td>`/`<th>`, and native row groups); manual mode reads `focusgrouprow` markers and each marked row's direct element children. The grid then infers `grid`, `row`, and `gridcell` roles one-way on generic elements, following the shared [minimum-role precedence](#behavior--role-mapping-and-precedence). Explicit roles and non-generic native semantics are preserved and never overridden, and they do not redefine topology. A native `<table>` continues to map through HTML-AAM; authors add `role="grid"` when they want the composite `grid` role exposed. The initial model does not support `treegrid`.
...
Authors use `grid` only when row and column relationships remain meaningful independently of layout. The initial model validates only the mounted flat-tree topology and requires it to be rectangular and spanless:
...
Grid topology is a structural property of the HTML, not a projection of the accessibility tree. The user agent classifies rows and cells from the markup — a native table in [automatic mode](#automatic-semantic-topology), or `focusgrouprow` markers and their direct flat-tree element children in [manual mode](#manual-row-enrollment) — and never reads authored ARIA roles or `ElementInternals` defaults to decide the topology. The `grid`, `row`, and `gridcell` roles are a one-way output of that structure, applied under the shared [minimum-role precedence](#behavior--role-mapping-and-precedence); explicit roles and non-generic native semantics are preserved but do not redefine topology.
...
`focusgroup="grid"` discovers every row and cell from a native HTML table structure. Native table sections (`<thead>`, `<tbody>`, `<tfoot>`) group rows, `<tr>` elements are rows, and `<td>`/`<th>` elements are cells. The user agent reads this structure directly; it does not consult authored ARIA roles or `ElementInternals` to build the topology.
...
`focusgrouprow` is structural: it never establishes a focusgroup owner or definition, nested scope, focusgroup segment, or memory. On a generic element it contributes a one-way inferred `row` role under the shared [minimum-role precedence](#behavior--role-mapping-and-precedence); on an element with explicit or non-generic native `row` semantics, that existing role is preserved. Cells are not marked with a `focusgroupcell` attribute: each direct element child of a marked row is a cell coordinate, and a generic cell is exposed one-way as `gridcell`.

The `focusgrouprow` marker locates rows explicitly, so a marked row may sit beneath nonfocusable generic wrappers in the owner's flat-tree subtree. Each cell, however, is a direct flat-tree element child of its marked row; manual mode does not search through cell wrappers. Focusable or otherwise non-generic wrappers around a row remain structural barriers so keyboard topology cannot diverge from the accessible grid.
...
- The generic owner is exposed one-way as `grid`, each `focusgrouprow` element as `row`, and each row's direct child as `gridcell`. Authored roles or non-generic native semantics, when present, are preserved instead.
...
- Manual mode makes row enrollment explicit for markup that has no native table structure; it never reads ARIA to build the topology.
...
Because `grid` implies [`itemcontrols`](#v2-itemcontrols), a grid's outer items are its cell/coordinate targets — a focusable cell boundary or the cell's selected target — and only the active cell's controls join sequential focus navigation. A cell with multiple controls uses a focusable cell boundary as its grid target, and its inner controls use `focusgroup="none"` so they do not become grid targets. Authors do not need to spell `itemcontrols`; a redundant explicit `itemcontrols` remains valid.
...
- `grid` implies `itemcontrols`, so the modifier need not be spelled. Directional navigation moves between cell targets, and Tab from the active cell enters its associated controls.
...
Every row has the same number of cells, so the initial shape has no missing coordinate slots. By default both axes have hard edges; the [wrapping and flow modifiers](#grid-wrapping-and-flow) can wrap movement to the opposite end of the same row or column, or flow it into an adjacent row or column. At a hard edge, or when a wrap or flow scan finds no other eligible target in the requested direction, focus does not move and the user agent does not consume the key for grid focusgroup behavior.
...
A `focusgrouprow` marker on an element with non-generic, non-`row` semantics, a malformed row or cell relationship, multiple eligible descendants in a nonfocusable cell, a focusable non-target descendant, an incompatible owner role, or an unsupported shape (ragged, spanning, or otherwise non-rectangular) makes the grid invalid. In that state, **grid focusgroup behavior is ignored**: directional key handling, memory, sequential-entry collapse, segment behavior, and any focusgroup inference for that grid do not run.
...
7. Use `grid` only when row and column relationships are semantically meaningful, not merely a visual grid layout. Automatic mode reads a native table; add `role="grid"` to expose grid semantics. Responsive card lists and masonry layouts stay lists and use linear navigation.
8. Use plain `focusgroup="grid"` for a native table, where rows and cells come from the table structure. Do not add `focusgrouprow` to an automatic grid.
9. Use `focusgroup="grid manual"` for generic or custom markup. Mark every semantic row with `focusgrouprow`; each marked row's direct flat-tree element children are its cells.
10. Let the grid infer `grid`, `row`, and `gridcell` on generic elements, or preserve explicit and native semantics where present. Do not mark cells with `focusgroupcell`, and do not use focusability to identify cells.
11. Keep the initial grid rectangular, spanless, and fully rendered. Do not use `grid` for ragged, spanning, virtualized, indexed, or `treegrid` structures.
12. Give a multi-control cell a focusable cell boundary and mark its inner controls with `focusgroup="none"`. `grid` implies `itemcontrols`, so only the active cell's controls participate in sequential focus navigation.
13. A grid has hard edges by default. Add `wrap` (or `row-wrap`/`col-wrap`) to wrap within a row or column, or `flow` (or `row-flow`/`col-flow`) to continue into the adjacent row or column. `nowrap` forces hard edges.
...
6. A single atomic behavior token for manual grids: expresses manual topology as one dedicated behavior token rather than a `grid` behavior plus a `manual` modifier. Not adopted. `manual` is instead a modifier (`focusgroup="grid manual"`), where `grid` is the behavior and `manual` is scoped to grid today and reusable by future behaviors, so it composes with the shared parser rather than minting a second behavior token. A user agent that recognizes `grid` but not `manual` ignores the modifier under the [token-list fallback](#index-of-focusgroup-values) and applies automatic grid; authors who depend on manual topology feature-detect `supports('manual')`. This trade-off is accepted in exchange for a reusable modifier and a single grid behavior token.
7. Focusgroup values for both rows and cells: duplicates row and cell identity in the attribute. Not adopted because automatic mode discovers both from the native table structure, while manual mode needs only explicit row enrollment; a marked row's direct flat-tree element children are its cells.
...
| Both axes derived from semantic row and column topology | (no direction modifier) | `grid` |
| Inline direction | `inline` | Linear behaviors |
| Block direction | `block` | Linear behaviors |

***Focusgroup wrapping and flow:***

| Description | HTML syntax | Applies to |
|---|---|---|
| Behavior default (hard edges on a grid) | (unspecified value) | Linear and grid behaviors; the `feed` wrapping model remains open |
| Wrap end→start (grid: within the same row and column) | `wrap` | Linear and grid (grid = `row-wrap col-wrap`) |
| No wrap / hard edges (explicit, overrides default) | `nowrap` | Linear and grid |
| Flow into the adjacent row and column | `flow` | `grid` (= `row-flow col-flow`) |
| Wrap within the same row | `row-wrap` | `grid` |
| Wrap within the same column | `col-wrap` | `grid` |
| Flow across rows | `row-flow` | `grid` |
| Flow across columns | `col-flow` | `grid` |

On a grid, `nowrap` forces hard edges and overrides `wrap`/`flow`. Conflicting same-axis grid modifiers (`row-wrap` with `row-flow`, or `col-wrap` with `col-flow`) are an author error and fall back to a hard edge. The `feed` wrapping model remains open; the current draft accepts neither wrapping token for `feed`.
