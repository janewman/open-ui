...
| [`itemcontrols`](#v2-itemcontrols) | V2 modifier; implied by `feed` | Adds the `focusgroup="none"` controls of the outer item that currently holds focus to sequential focus navigation, and skips controls in items that aren't focused. |
...
While focus is inside an `itemcontrols` focusgroup, the **active outer item** is the outer item that contains current focus: the focused outer item itself, or the nearest containing outer item when focus is on one of its associated `focusgroup="none"` controls. Activity follows current focus rather than a separately stored selection, so when focus leaves the group no item is active. The active outer item is distinct from the optional last-focused memory, which persists after focus leaves and can select an item on the next entry.
...
3. Focus landing on that item, or on one of its controls, makes it the active outer item. Its associated controls follow the boundary in sequential order, while other outer items and the controls associated with inactive items stay out of sequential focus navigation.
...
If focus lands on a descendant of another eligible outer item, that item is active. If focus leaves every outer item, no item is active until the next directional or sequential entry operation selects one. Controls owned by the removed item are no longer exposed through the `itemcontrols` extension. Independently scoped, opted-out, and top-layer descendants retain their existing behavior.
