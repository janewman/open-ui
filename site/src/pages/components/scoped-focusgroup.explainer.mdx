...
| [`grid`](#automatic-semantic-topology) | V2 2-D behavior | Discovers rows and cells automatically from the owner's existing grid semantics. |
| [`grid-manual`](#manual-row-enrollment) | V2 2-D behavior | Requires authors to enroll every semantic row with the separate `focusgrouprow` attribute. |
| [`itemcontrols`](#v2-itemcontrols) | V2 modifier; implied by `feed` | Adds controls associated with the active outer item to sequential focus navigation and skips controls associated with inactive items. |
...
Focusgroup **V1** is the current linear design. It narrows the scope of the [original focusgroup explainer](https://github.com/openui/open-ui/blob/fdd348a/site/src/pages/components/focusgroup.explainer.mdx). **V2** covers the active new work — grid, `itemcontrols`, and `feed`. This draft also defines the [`itemcontrols`](#v2-itemcontrols) modifier for the controls inside a focused item and the [`feed`](#v2-feed-focusgroups) behavior for ARIA-feed-style content streams.
...
The [ARIA in HTML specification](https://www.w3.org/TR/html-aria/#docconformance) defines which ARIA roles authors may use on each HTML element. Its [`button` entry](https://www.w3.org/TR/html-aria/#el-button) lists roles including `tab`, `radio`, the `menuitem` variants, and `gridcell` (currently marked there as a proposed addition). The focusgroup button exception follows that per-element list: it never infers a role that the specification does not allow on `<button>`, such as `article` or `row`.

V1 linear focusgroup behavior tokens (APG alignment and minimum roles). All six produce the same linear navigation semantics — only semantic expectations, child inference, and default modifiers differ. The V2 `feed` row maps to the ARIA feed structure, and the V2 grid row is retained for the detailed grid design below:
...
| `feed` (V2) | [Feed](https://www.w3.org/WAI/ARIA/apg/patterns/feed/) | [`feed`](https://www.w3.org/TR/wai-aria-1.2/#feed) | [`article`](https://www.w3.org/TR/wai-aria-1.2/#article) | `block` `itemcontrols` | [APG Feed](https://www.w3.org/WAI/ARIA/apg/patterns/feed/) |

V2 grid and item-controls tokens and modifiers:
...
| `itemcontrols` | Active-item controls in sequential navigation | Not applicable | None | [V2 itemcontrols](#v2-itemcontrols) |
...
The [`itemcontrols`](#v2-itemcontrols) modifier changes how a focusgroup treats controls inside an item; it does not define another navigation topology.
...
The `itemcontrols` behavior adds a scoped exception for `focusgroup="none"` descendants of an outer focusgroup item. Those descendants remain excluded from directional navigation, but the controls associated with the [active outer item](#active-outer-item) participate in sequential focus navigation. Equivalent controls associated with inactive outer items do not create sequential entry points. `feed` implies this behavior.

Each segment operates independently for certain behaviors:
- **Guaranteed tab stop:** Each segment ensures exactly one tab-accessible focusgroup item. The [`itemcontrols`](#v2-itemcontrols) extension additionally exposes `focusgroup="none"` controls associated with the active item.
...
The [`itemcontrols`](#v2-itemcontrols) extension defines additional sequential access separately.
...
- [`itemcontrols`](#v2-itemcontrols) - preserve an outer focusgroup item as the directional-navigation target while managing sequential access to its `focusgroup="none"` controls. The `feed` behavior implies this modifier.
...
Note that the following only apply to linear `focusgroup` definitions. Both grid behaviors derive both axes from row and column structure and do not accept `inline` or `block`; the `feed` key model remains open.
...
The `itemcontrols` changes to entry, sequential navigation, and memory are defined in the [`itemcontrols` extension](#sequential-entry-extension).
...
The selected behavior determines outer items before `itemcontrols` changes sequential navigation. Linear behaviors use their managed focusgroup items; the `feed` behavior uses its top-level article items. Each opted-out control is then associated with its nearest containing outer item.
...
The grid key set is defined in [Grid directional navigation](#grid-directional-navigation). Home and End move to the first and last eligible targets in the current row. Ctrl+Home and Ctrl+End move to the first and last eligible targets in row-major order. Grid focusgroup behavior does not handle Page Up or Page Down.

The grid and feed behaviors do not change selection, activation, expansion, or content-loading behavior.
...
5. Add `itemcontrols` to a behavior when each item contains nested controls that should stay out of directional navigation. Mark those controls with `focusgroup="none"`; only controls associated with the active item participate in sequential focus navigation.
6. Use generic containers with `focusgroup="feed"` to let the behavior infer the `feed` and `article` roles. Include `aria-labelledby`, `aria-posinset`, and `aria-setsize` on each article per the [APG feed pattern](https://www.w3.org/WAI/ARIA/apg/patterns/feed/). Mark nested controls with `focusgroup="none"`; `feed` implies `itemcontrols`, so only controls associated with the active item participate in sequential focus navigation.
7. Use a grid behavior only when the owner already exposes `grid` semantics and rows and columns remain semantically meaningful after layout changes. A plain table requires `role="grid"`; responsive lists and masonry layouts use linear navigation instead.
8. Use `grid` when semantic row and cell ownership is automatic and unambiguous. Do not add `focusgrouprow` to an automatic grid.
9. Use the atomic `grid-manual` behavior for arbitrary or custom markup. Mark every owned semantic row with `focusgrouprow`; do not rely on automatic fallback.
10. Preserve native, explicit, or custom-element default row and cell semantics in either mode. Do not mark cells with `focusgroupcell`, and do not use focusability to identify cells.
11. Keep the initial grid rectangular, spanless, and fully rendered. Do not use the grid behaviors for ragged, spanning, virtualized, indexed, or `treegrid` structures.
12. Give a multi-control cell a focusable cell boundary. Mark its inner controls with `focusgroup="none"` and add `itemcontrols` when only the active cell's controls should participate in sequential focus navigation.
...
Behavior tokens: See the earlier Supported behaviors mapping table for the definitive list, minimum container roles, child inference notes, and [default modifiers](#default-modifiers). Explicit container and child roles always override any inference. Explicit modifiers always override default modifiers. `grid` and `grid-manual` are distinct atomic behavior tokens; grid behaviors perform no role inference and require compatible authored or default semantics.

***Proposed grid behaviors:***

| Description | HTML syntax |
|---|---|
| Static 2-D grid with automatic semantic topology discovery | `grid` |
| Static 2-D grid with explicit semantic row enrollment | `grid-manual` |

`focusgrouprow` is not a token:

| Description | HTML syntax |
|---|---|
| Boolean marker on every semantic row owned by `grid-manual`; reflected as `focusGroupRow` | `focusgrouprow` |

***V2 feed and item controls:***
